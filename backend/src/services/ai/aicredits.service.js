...
export class AiCreditsService {
    ...
    static async generateLeadScore(leadData, conversationHistory) {
        try {
            const response = await axios.post('https://api.aicredits.in/v1/chat/completions', // Assuming standard OpenAI compatible endpoint
            {
    ...
    static async summarizeConversation(conversationHistory) {
        try {
            const response = await axios.post('https://api.aicredits.in/v1/chat/completions', {
                model: 'gpt-4o-mini',
                messages: [
                    {
                        role: 'system',
