...
export class UsersController {
  ...
  static async update(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const user = await UsersService.updateUser(req.user!.organizationId, req.params.id as string, req.body);
  ...
  static async deactivate(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      await UsersService.deactivateUser(req.user!.organizationId, req.params.id as string);
