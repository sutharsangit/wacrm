...
export default function LoginPage() {
  ...
  const { login, theme, setTheme } = useAuthStore();
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4 relative transition-colors duration-300">
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <Link href="/forgot-password" className="text-[10px] text-primary hover:underline font-semibold">Forgot password?</Link>
            </div>
            <div className="relative">
              <input
                ...
                disabled={loading}
                placeholder="••••••"
                className={`w-full bg-secondary border ${errors.password ? 'border-destructive' : 'border-border'} rounded-lg pl-3.5 pr-10 py-2.5 text-xs text-foreground focus:outline-none focus:border-primary transition-colors`}
                {...register('password')}
              />
              <button
                type="button"
        ...
        <div className="mt-6 text-center text-xs text-muted-foreground">
          New to WACRM?{' '}
          <Link href="/register" className="text-primary hover:underline font-semibold">Create a free account</Link>
        </div>
      </div>
