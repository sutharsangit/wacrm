...
import { Sparkles, Eye, EyeOff, Loader2, Sun, Moon } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const { register: authRegister, updateOnboardingStatus, theme, setTheme } = useAuthStore();
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                disabled={loading}
                placeholder="Marcus"
                className={`w-full bg-secondary border ${errors.name ? 'border-destructive' : 'border-border'} rounded-lg px-3.5 py-2 text-xs text-foreground placeholder-zinc-500 focus:outline-none focus:border-primary transition-colors`}
                {...register('name')}
              />
              {errors.name && (
            ...
            <div>
              ...
              <input
                ...
                className={`w-full bg-secondary border ${errors.businessName ? 'border-destructive' : 'border-border'} rounded-lg px-3.5 py-2 text-xs text-foreground placeholder-zinc-500 focus:outline-none focus:border-primary transition-colors`}
          ...
          <div>
            ...
            <select
              disabled={loading}
              className={`w-full bg-secondary border ${errors.industry ? 'border-destructive' : 'border-border'} rounded-lg px-3.5 py-2 text-xs text-foreground focus:outline-none focus:border-primary transition-colors`}
              {...register('industry')}
            >
          ...
          <div>
            ...
            <input
              ...
              className={`w-full bg-secondary border ${errors.email ? 'border-destructive' : 'border-border'} rounded-lg px-3.5 py-2 text-xs text-foreground placeholder-zinc-500 focus:outline-none focus:border-primary transition-colors`}
              {...register('email')}
            />
            {errors.email && (
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                disabled={loading}
                placeholder="••••••"
                className={`w-full bg-secondary border ${errors.password ? 'border-destructive' : 'border-border'} rounded-lg pl-3.5 pr-10 py-2 text-xs text-foreground focus:outline-none focus:border-primary transition-colors`}
                {...register('password')}
              />
              <button
                type="button"
