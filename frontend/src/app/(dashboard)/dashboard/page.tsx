...
export default function DashboardOverview() {
  ...
  const { leads, fetchLeads, addLead } = useLeadsStore();
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-card border border-border p-5 rounded-xl hover:border-primary/30 transition-all flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white leading-none">{totalLeadsCount}</h3>
        ...
        <div className="bg-card border border-border p-5 rounded-xl hover:border-primary/30 transition-all flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white leading-none">{qualifiedLeadsCount}</h3>
            <span className="text-[10px] text-primary font-medium flex items-center gap-1 mt-1">
        ...
        <div className="bg-card border border-border p-5 rounded-xl hover:border-primary/30 transition-all flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white leading-none">{pendingFollowUpCount}</h3>
        ...
        <div className="bg-card border border-border p-5 rounded-xl hover:border-primary/30 transition-all flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white leading-none">{conversionRate}%</h3>
        ...
        <div className="bg-card border border-border p-5 rounded-xl hover:border-primary/30 transition-all flex flex-col justify-between h-32 relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white leading-none">$31,200</h3>
            <span className="text-[10px] text-primary font-medium flex items-center gap-1 mt-1">
        ...
        <div className="bg-card border border-border rounded-xl p-5 flex flex-col justify-between">
          ...
          <div className="h-44 w-full relative flex items-center justify-center my-3">
            ...
            <div className="absolute text-center">
              ...
              <span className="text-lg font-bold text-white mt-1 block">830</span>
            </div>
          </div>
        ...
        <div className="relative border-l border-zinc-800 ml-3 pl-6 space-y-5">
          {allActivities.map((activity) => (
            <div key={activity.id} className="relative group">
              ...
              <div className="absolute -left-[30px] top-1.5 w-2 h-2 rounded-full bg-zinc-700 border-2 border-zinc-950 group-hover:bg-primary group-hover:scale-125 transition-all" />
              
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <div>
                  <span className="text-xs font-semibold text-white">{activity.title}</span>
      ...
      {showAddLead && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-6 max-w-md w-full relative animate-fade-in">
            ...
            <form onSubmit={handleCreateLead} className="mt-4 grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-[9px] uppercase font-bold text-zinc-400 block mb-1">Lead Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-zinc-400 block mb-1">Company</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-[9px] uppercase font-bold text-zinc-400 block mb-1">Acquisition Source</label>
                <select
                  value={newLeadForm.source}
                  onChange={(e) => setNewLeadForm({ ...newLeadForm, source: e.target.value as any })}
                  className="w-full bg-zinc-900 border border-zinc-800 rounded p-2 text-xs text-zinc-300 focus:outline-none focus:border-primary"
                >
