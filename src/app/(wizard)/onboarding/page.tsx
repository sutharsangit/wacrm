...
export default function OnboardingWizard() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col justify-between py-12 px-4 sm:px-6 lg:px-8">
      {/* Brand Header */}
      ...
      {/* Main Container */}
      <div className="max-w-xl mx-auto w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-8 shadow-2xl relative">
        {/* Step Indicator */}
          ...
          {[
            ...
            return (
              <div key={item.step} className="flex flex-col items-center gap-1.5 flex-1 relative">
                ...
                <span className={`text-[10px] font-semibold tracking-wider uppercase ${isActive ? 'text-primary' : 'text-zinc-500'}`}>
                  {item.label}
                </span>
              </div>
            );
        ...
        {currentStep === 1 && (
          <div className="space-y-5 animate-fade-in">
            <div>
              ...
              <p className="text-xs text-zinc-400">Configure your initial CRM profile workspace settings.</p>
            </div>
            
            <div className="space-y-4">
              <div>
                <label className="text-[10px] uppercase font-bold tracking-wider text-zinc-400 block mb-1">Business Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-primary transition-colors"
                />
              </div>

              <div>
                <label className="text-[10px] uppercase font-bold tracking-wider text-zinc-400 block mb-1">Company Size</label>
                <select
                  value={companyForm.size}
                  onChange={(e) => setCompanyForm({ ...companyForm, size: e.target.value })}
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-xs text-zinc-300 focus:outline-none focus:border-primary transition-colors"
                >
              ...
              <div>
                <label className="text-[10px] uppercase font-bold tracking-wider text-zinc-400 block mb-1">Company Website</label>
                <input
                  type="url"
        ...
        {currentStep === 2 && (
          <div className="space-y-5 animate-fade-in">
            <div>
              ...
              <p className="text-xs text-zinc-400">Select where your customer inquiries originate from.</p>
            </div>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <span className="text-[10px] text-zinc-500 block mt-0.5">{source.description}</span>
                    </div>

                    {isComingSoon ? (
                      <span className="inline-flex items-center gap-1 text-[9px] font-bold text-zinc-500 bg-zinc-900 border border-zinc-800 px-2 py-1 rounded">
                      ...
                      <button
                        ...
                        className={`text-[10px] font-bold px-3 py-1.5 rounded transition-colors ${
                          isConnected 
                            ? 'bg-zinc-900 text-primary border border-primary/30 hover:bg-zinc-900/80' 
        ...
        {currentStep === 3 && (
          <div className="space-y-5 animate-fade-in">
            <div>
              ...
              <p className="text-xs text-zinc-400">Start with a 14-day free trial. Downgrade or upgrade at any time.</p>
            </div>

            {loading ? (
              <div className="py-12 flex flex-col items-center justify-center gap-3">
                <Loader2 className="w-8 h-8 text-primary animate-spin" />
                <span className="text-xs text-zinc-400 font-medium">Provisioning secure WhatsApp gateway...</span>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-4.5 h-4.5 rounded-full border flex items-center justify-center ${
                          isSelected ? 'border-primary text-primary' : 'border-zinc-500'
                        }`}>
                          {isSelected && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
                        </div>
                        <div>
                          <span className="font-bold text-xs block">{plan.name}</span>
                          <span className="text-[10px] text-zinc-500 block mt-0.5">{plan.limit}</span>
                        </div>
                      </div>
                      <span className="font-bold text-sm">{plan.price}<span className="text-[9px] text-zinc-500 font-normal">/mo</span></span>
                    </div>
                  );
                })}
              </div>
        ...
        {currentStep === 4 && (
          <div className="text-center py-6 animate-fade-in space-y-4">
            ...
            <div className="border border-zinc-800 rounded-xl p-4 bg-zinc-900/20 text-left max-w-sm mx-auto space-y-2">
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider block">Connected sources</span>
              <div className="flex gap-2 flex-wrap">
                {connectedSources.length > 0 ? (
                  ...
                  <span className="text-[10px] text-zinc-500">None connected (Manual Add only)</span>
                )}
              </div>
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider block pt-1">Provisioned Plan</span>
              <span className="text-xs font-semibold text-white block">{selectedPlan} Tier ($0 due now)</span>
            </div>
          </div>
        )}
