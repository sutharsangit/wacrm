...
7. **WhatsApp Module (`/whatsapp`)**:
   - A complete WhatsApp Business CRM Integration workspace including a nested navigation menu:
     - **Overview (`/whatsapp/overview`)**: Business metrics dashboard showing Connection status, numbers, tier, limits, daily activity area charts, and recent inbound message logs.
     - **Connect Number (`/whatsapp/connect`)**: Onboarding options selector (Existing App vs Cloud API Number) and a multi-step Connection Wizard (Business Details, GST/Website upload verification, SMS OTP simulator, and Review).
     - **Inbox (`/whatsapp/inbox`)**: Multi-pane conversational panel featuring thread search, AI Copilot toggle, template quick inserts, note writers, customer assignment dropdowns, follow-up calendar logs, and lead statuses update buttons.
     - **Broadcast (`/whatsapp/broadcast`)**: Marketing campaign creator allowing audience targeting, CSV file uploads, message template selects with live previews, scheduling, and metrics history logs.
     - **AI Qualification (`/whatsapp/qualification`)**: Drag-and-drop conversational screening builder with type selectors (Text, Option, Dropdown, Yes/No, Number) and scoring rules mapping.
     - **Templates (`/whatsapp/templates`)**: Standard list filterable by category (Marketing, Utility, Authentication) with a template builder featuring a live simulated mobile phone preview.
     - **Contacts (`/whatsapp/contacts`)**: Synchronized contacts grid with tags list, lead sources, and bulk action checkboxes (tagging, deletion).
     - **Campaigns (`/whatsapp/campaigns`)**: Analytics center with Recharts area/funnel/bar metrics charts.
     - **Settings (`/whatsapp/settings`)**: Configures profile info, webhook listener URL, verify tokens, API keys, and notification triggers.
